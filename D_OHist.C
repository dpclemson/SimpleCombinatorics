...
void callHK(int H,int K){
  ...
  TProfile* histogram = (TProfile*)file->Get(Form("d_hmult_recursion_0_%d",K-2));
  if ( histogram == NULL ){
  ...
  tex.DrawLatex(0.6,0.6,Form("(k-1)!=%d",(int)TMath::Gamma(K)));
  ...
  c1.Print(Form("Figures/TestHist_double_h%d_k%d_residuals.png",H,K));
